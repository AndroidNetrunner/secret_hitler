...
const assignRoles = (currentGame: Game_room): Map<User, Role> => {
  ...
  const possibleRoles = currentGame.mastermind
    ? setMastermind(
        roleByNumberOfPlayers[numberOfPlayers as 5 | 6 | 7 | 8 | 9 | 10],
        currentGame
      )
    : ([
        ...roleByNumberOfPlayers[numberOfPlayers as 5 | 6 | 7 | 8 | 9 | 10],
...
const getWhoAreFascists = (
  ...
  roleOfPlayers.forEach((role, opponent) => {
    ...
    if (role === FASCIST) fascists.push(opponent);
  });
...
const getWhoIsHitler = (roleOfPlayers: Map<User, Role>): User => {
  for (let [opponent, role] of roleOfPlayers)
    if (role === HITLER) return opponent;
  throw new Error("No hitler in roles");
};

const decideDescriptionByRole = (
  player: User,
  roleOfPlayers: Map<User, Role>
): string => {
  ...
  switch (role) {
    case HITLER:
      if (roleOfPlayers.size > 6)
        ...
        return `당신의 파시스트 동료는 ${getWhoAreFascists(
          player,
          roleOfPlayers
        )}입니다!`;
    case LIBERAL:
    ...
    case FASCIST:
      return `당신의 파시스트 동료는 ${getWhoAreFascists(
        player,
        roleOfPlayers
      )}이며, 히틀러는 ${getWhoIsHitler(roleOfPlayers)}입니다!`;
    case MASTERMIND:
...
const selectBoard = (currentGame: Game_room): void => {
  const numberOfPlayers = currentGame.gameStatus.players.length as
    | 5
    | 6
    | 7
    | 8
...
const printBoard = (currentGame: Game_room): void => {
  ...
  if (!fascistBoard) return;
  const embed = new MessageEmbed()
    .setTitle("모든 플레이어에게 역할 배분을 하였습니다.")
    .setFields({
