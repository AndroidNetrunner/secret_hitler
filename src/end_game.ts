...
export const completeFascistTrack = (currentGame : Game_room) : void => {
    const description = "파시스트 법안 트랙 완성으로 인한 파시스트당 승리";
    endGame(currentGame, description);
}

export const completeLiberalTrack = (currentGame : Game_room) : void => {
    const description = `자유당 법안 트랙 완성으로 인한 자유당 승리`;
    endGame(currentGame, description);
}

export const executeHitler = (currentGame : Game_room) : void => {
    const description = `히틀러 처형으로 인한 자유당 승리`;
    endGame(currentGame, description);
}

export const electHitlerByChancellor = (currentGame : Game_room) : void => {
    const description = `히틀러의 수상 당선으로 인한 파시스트당 승리`;
    endGame(currentGame, description);
}

export const makeSuddenDeathByMastermind = (currentGame: Game_room) : void => {
    const description = `4번째 자유당 법안 제정 후 5번째 파시스트 법안 제정으로 인한 배후 승리`;
    endGame(currentGame, description);
}

export const enactFourthLiberalPolicyByMastermind = (currentGame: Game_room) : void => {
